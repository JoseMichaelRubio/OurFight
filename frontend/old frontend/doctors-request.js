...
xhr.open("GET", "http://20.62.192.255/doctors"); //method and the url 
...
function renderItemsInTable(doctorsTable){
    ...
    for(let doctor of doctorsTable){
      ...
      newRow.innerHTML = `<td>${doctor.id}</td><td>${doctor.name}</td><td>${doctor.specialty}</td><td>${doctor.gender}</td><td>${doctor.available}</td><td>${doctor.volunteer}</td>`;
